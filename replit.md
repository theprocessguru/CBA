...
## Recent Changes
- **Speaker Registration Architecture Verified (August 18, 2025)**: Confirmed AI Summit speaker system is working correctly. The 38 AI Summit registrations are general attendees who used the main registration form. Only 1 actual speaker interest submission exists (test entry). Fixed speaker registration endpoint to save speakers to both `ai_summit_speaker_interests` and main `ai_summit_registrations` tables with correct "speaker" roles for future submissions. Admin login issue resolved - system now works properly in Replit environment.
