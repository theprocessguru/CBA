...
## Recent Changes
- **AI Summit Schedule Restructured (August 18, 2025)**: Changed from 10 half-hour slots to 5 one-hour presentation slots as requested. New schedule: 11am-12pm Opening Keynote, 12pm-1pm Workshop, 1pm-2pm Lunch Break, 2pm-3pm Marketing Workshop, 3pm-4pm Panel Discussion. Built complete speaker self-service slot selection system at /speaker-slot-selection with booking validation, profile checks, and availability tracking. Only 5 total presentation slots available with mostly 1-hour duration. **Updated with realistic timing**: Added 10-minute room transitions (5 min entry + 5 min exit) and Q&A sessions after each presentation. Schedule now includes 50-minute keynote/workshop slots (40 min presentation + 10 min Q&A) and 25-minute demo/talk slots (20 min presentation + 5 min Q&A).
- **Speaker Recovery System Implemented (August 18, 2025)**: Built comprehensive Speaker Recovery Tool at `/admin/speaker-recovery` to solve missing speaker mystery. Confirmed user is the confirmed speaker for AI Summit. Discovered potential speakers registered as general attendees instead of using Speaker Interest form after Ben's radio appearance (August 11-18). Identified high-profile potential speakers: Calvin (CEO EbonyClass), Mark Waterstone (CEO Rosepride), Jo Chesney (CEO TBXH), Dahna-Esther Fearon (Founder Release Me), Steve Ball (Process Guru). Corrected team member classifications: Rithu and Ranjani updated from attendees to team members (were testing app). Recovery tool enables conversion of attendees to speakers with full talk details and database updates. Removed test data (Dr. Sarah Johnson) from speaker database.
- **Speakers Page Database Mapping Fixed (August 18, 2025)**: Corrected AI Summit speakers endpoint to use actual database column names (name, email, job_title, linked_in, etc.) instead of non-existent speaker-prefixed columns. Removed redundant schema definitions for speaker_name, speaker_email fields that didn't exist in database. Updated Drizzle schema to match actual database structure.
- **Administrator Management Enhanced (August 18, 2025)**: Added comprehensive admin editing functionality including edit details, reset password, and remove admin capabilities. Each administrator now has dropdown menu with actions. Implemented safety features preventing self-deletion and enforcing @croydonba.org.uk email domain validation.
